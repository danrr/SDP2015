...
def blocked_if_turn_by_angle(world, angle):
    '''
    Checks if our defender can pass the ball if turned by angle first
    '''
    angle_to_turn = angle
    predicted_y = predict_y_intersection(
        world,  world.their_attacker.x, world.our_defender, angle_to_turn, full_width=False, bounce=True)
    if predicted_y is None:
        return True
    return abs(predicted_y - world.their_attacker.y) < world.their_attacker.length
...
def predict_y_intersection_after_turn(world, predict_for_x, robot, angle_to_turn, full_width=False, bounce=False):
    """
    Predicts the (x, y) coordinates of the ball shot by the robot
    Corrects them if it's out of the bottom_y - top_y range.
    If bounce is set to True, predicts for a bounced shot
