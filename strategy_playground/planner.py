...
class Planner:
    ...
    def __init__(self, our_side, pitch_num):
        ...
        """
        Previous code has lists of strategies for each state but only ever
        chose the first
        """
        #Strategies the attacker can choose
        self._attacker_strategies = {'defence' : AttackerDefend,
                                     'grab' : AttackerGrab,
                                     'score' : AttackerShoot,
                                      'Pass' : AttackerPass}

        #Strategies the defender can choose
        self._defender_strategies = {'defence' : DefenderDefence,
                                     'grab' : DefenderGrab,
                                     'pass' : DefenderPass}

        #Both start in the defencive strategy
    ...
    def choose_attacker_strategy(self, world):
        next_strategy = self._attacker_strategies[self._attacker_state]
        return next_strategy(world)

    # Provisional. Choose the first strategy in the applicable list.
    def choose_defender_strategy(self, world):
        next_strategy = self._defender_strategies[self._defender_state]
        return next_strategy(world)
